...
if [ -z "$PROVISIONING" ]
then
    if ! does_snapshot_exist "NcVM-installation" && false
    
#     yesno_box_no "Do you want to use LVM snapshots to be able to restore your root partition during upgrades and such?
...
then
    if [ -n "$PROVISIONING" ] || true
    # yesno_box_yes "Do you want to make all free space available to your root partition?"
    then
